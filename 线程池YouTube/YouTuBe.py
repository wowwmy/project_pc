...
class YTB:
    ...
    def request(self, search, first=False, continuation=''):
        ...
        :return: 网页script中的 wph_js 数据和 token
        """
        if first:
            first_jsons = {
                'search_query': search
    ...
    def continue_spider(self, num):
        ...
        if self.redis_sql.scard('url') != 0:
            logging.info('开启断点续爬')
            self.continue_spider(num)
        else:
            logging.info('数据已经采集完毕，无需进行进行断点续爬')
        ...
        with futures.ThreadPoolExecutor(max_workers=num) as executor:
            ...
            while True:
                ...
                if self.redis_sql.scard('url') != 0:
                    ...
                    self.redis_sql.sadd('url_check', json.dumps(video_id))
                    self.drop_video_id(json.dumps(video_id), 'url')
                else:
                    break
